...
export async function GET(request: NextRequest) {
    try {
        ...
        let query = adminClient.from("clients").select("*").eq("has_invoicing", true).order("created_at", { ascending: false })
        ...
        if (clientsError) {
            console.error("Error fetching invoicing clients:", clientsError)
