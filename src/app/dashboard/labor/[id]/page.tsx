...
export default function LaborClientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-background to-green-100/50">
      <header className="border-b border-border bg-background/80 backdrop-blur-md sticky top-0 z-10">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center gap-4">
            <Link href={role === "cliente" ? "/dashboard" : "/dashboard/labor"}>
              <Button variant="ghost" size="icon">
                <ArrowLeft className="h-5 w-5" />
              </Button>
            </Link>
            <div>
              <h1 className="text-2xl font-bold">Laboral - {client.name}</h1>
      ...
      <main className="container mx-auto px-6 py-8 space-y-8">
        ...
        <LaborSection clientId={client.id} userRole={role} />
      </main>
    </div>
  )
}
